...
# Database Models - Simplified User model without email
class User(Base):
    __tablename__ = "users"
...
# Pydantic models - Simplified without email
...
# API Routes - Simplified registration without email
...
async def upload_face(
    ...
    # Remove any existing face data for this user
    existing_face_data = db.query(FaceData).filter(FaceData.user_id == current_user.id).all()
    for data in existing_face_data:
        db.delete(data)
...
async def face_login(request: FaceLoginRequest, db: Session = Depends(get_db)):
    try:
        ...
        if best_match_distance < 0.6:  # Lower is more strict
            ...
            if user and user.face_registered:
                # Create access token
                access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
                access_token = create_access_token(
                    data={"sub": user.username}, expires_delta=access_token_expires
...
async def clear_face_data(
    ...
    return {"message": "Face data cleared successfully"}
